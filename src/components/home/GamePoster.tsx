...
const Posters: Record<string, PosterDef> = {
  wordle: {
    w: 300,
    h: 200,
    content: () => {
...
export function GamePoster({ slug, className, style }: GamePosterProps) {
  const def = Posters[slug]
  if (!def) return <div className={className} style={style} />

  const scale = Math.min(TARGET_W / def.w, TARGET_H / def.h)
  const drawnW = def.w * scale
  const drawnH = def.h * scale
  const tx = (TARGET_W - drawnW) / 2
  const ty = (TARGET_H - drawnH) / 2
  ...
  return (
    <div className={className} style={{ ...wrapStyle, ...style }}>
      <svg
        ...
        style={svgStyle}
      >
        {def.bg && <rect width={TARGET_W} height={TARGET_H} fill={def.bg} />}
