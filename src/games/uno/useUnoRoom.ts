...
export interface UseUnoRoomReturn {
...
export function useUnoRoom(): UseUnoRoomReturn {
  ...
  function subscribeToRoom(code: string) {
    ...
    channelRef.current = supabase
      ...
      .on(
        'postgres_changes',
        { event: 'UPDATE', schema: 'public', table: 'uno_rooms', filter: `code=eq.${code}` },
        (payload) => {
          const parsed = GameStateSchema.safeParse(payload.new.state)
          if (!parsed.success) {
            console.error('[uno] Invalid GameState payload:', parsed.error)
            return
          }
  ...
  const createRoom = useCallback(async (playerName: string) => {
    ...
    const host: Player = { id, name: playerName, isHost: true }
    const initialState = createLobbyState(host)

    const { error: err } = await supabase.from('uno_rooms').insert({ code, state: initialState })
  ...
  const joinRoom = useCallback(async (code: string, playerName: string) => {
    ...
    if (!parsedCurrent.success) {
      console.error('[uno] Invalid GameState in joinRoom:', parsedCurrent.error)
    ...
    const id = crypto.randomUUID()
    const player: Player = { id, name: playerName, isHost: false }
  ...
  const restoreSession = useCallback(async () => {
    ...
    if (!parsedState.success) {
      console.error('[uno] Invalid GameState in restoreSession:', parsedState.error)
      clearSession()
      setSavedSession(null)
      setStatus('idle')
      return
      ...
      for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        ...
        if (attempt < MAX_RETRIES) {
          ...
          if (!parsedFresh.success) {
            console.error('[uno] Invalid GameState in dispatch retry:', parsedFresh.error)
            return
          }
