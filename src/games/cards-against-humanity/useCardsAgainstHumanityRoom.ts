...
export function useCardsAgainstHumanityRoom(): UseCahRoomReturn {
  ...
  const createRoom = useCallback(async (playerName: string) => {
    ...
    const { error: err } = await supabase.from(TABLE).insert({ code, state: initialState })
  ...
  const dispatch = useCallback(
    async (action: GameAction) => {
      ...
      await supabase.from(TABLE).update({ state: newState }).eq('code', roomCode)
    },
    [gameState, roomCode]
  )
